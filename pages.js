...
const Pages = {
  ...
  signalering() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">Signaleringen</div></div>
    <div class="content">
      <div class="card">
  ...
  medewerkers() {
    const mws = DB.getMedewerkers();
    return `
    <div class="topbar"><div class="topbar-title">Medewerkers</div>
  ...
  dossier() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">Personeelsdossier</div>
      <div class="flex gap2">
  ...
  handtekening() {
    ...
    const opts = mws.map(m=>`<option value="${m.id}">${H(mwNaam(m))}</option>`).join('');
    return `
    <div class="topbar"><div class="topbar-title">Handtekening & Paraaf</div></div>
    <div class="content">
  ...
  bijlagen() {
    ...
    const opts = mws.map(m=>`<option value="${m.id}">${H(mwNaam(m))}</option>`).join('');
    return `
    <div class="topbar"><div class="topbar-title">16 VCU Bijlagen</div>
  ...
  certificaten() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">Certificaten & VCA</div>
      <div class="flex gap2">
  ...
  incidenten() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">Incidenten & Bijna-Ongevallen</div>
  ...
  evaluaties() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">Evaluaties</div>
  ...
  handboek() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">VCU Handboek</div>
      <div class="flex gap2">
  ...
  inleners() {
    ...
    return `
    <div class="topbar"><div class="topbar-title">Inleners / Opdrachtgevers</div>
  ...
  rapporten() {
    ...
    const opts = mws.map(m=>`<option value="${m.id}">${H(mwNaam(m))}</option>`).join('');
    return `
    <div class="topbar"><div class="topbar-title">Afdrukken & Export</div></div>
    <div class="content">
      <div class="grid2">
  ...
  instellingen() {
    const inst = DB.getInstellingen();
    return `
    <div class="topbar"><div class="topbar-title">Instellingen</div></div>
    <div class="content">
...
function renderBijlagenGrid(mwId) {
  ...
  document.getElementById('bijlage-grid').innerHTML = BIJLAGEN.map((naam,i)=>{
    const b = bijl[i+1]||{};
    return `<div class="bijlage-card ${b.compleet?'done':''}" onclick="toggleBijlageGrid(${mwId},${i+1},this)">
      <div class="bijlage-num">${i+1}</div>
      <div style="flex:1;font-size:12px;">${naam}</div>
      ${b.compleet?'<i class="ti ti-circle-check" style="color:var(--green);font-size:16px;"></i>':'<i class="ti ti-circle-dotted" style="color:var(--text3);font-size:16px;"></i>'}
    </div>`;
  }).join('');
}

function toggleBijlageGrid(mwId, nr, card) {
  const b = DB.getBijlage(mwId, nr);
  const nieuw = !b.compleet;
  DB.saveBijlage(mwId, nr, { compleet: nieuw });
  renderBijlagenGrid(mwId);
  App.updateBadges();
}
